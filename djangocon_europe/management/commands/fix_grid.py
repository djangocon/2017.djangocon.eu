    ...
    def handle_noargs(self, **options):
        ...
        for plugin in plugins:
            ...
            if real_parent.pk in levels:
                real_parent.app_data.layout.extra_classes = real_parent.app_data.layout.extra_classes.replace(
                    'rowsponsor-row', 'row sponsor-row')
                real_parent.app_data.layout.extra_classes = real_parent.app_data.layout.extra_classes.replace(
                    'sponsor-row', 'sponsors-row')
                real_parent.save()
            if real_parent.pk in levels and levels[real_parent.pk] not in real_parent.app_data.layout.extra_classes:
                ...
                real_parent.save()
            if ('sponsors-row' in real_parent.app_data.layout.extra_classes and
                    len(plugin.app_data.layout.extra_classes) == 0):
